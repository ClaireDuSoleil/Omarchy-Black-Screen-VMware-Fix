#!/usr/bin/env bash
#
# post-install.sh -- finish the VM after install-kernel-patch.sh has rebooted it.
#
# Only needs to be run once, the first time you apply the patch -- everything here sets up
# the desktop environment, not the kernel, so later re-runs of install-kernel-patch.sh (e.g.
# after a system update replaces the kernel again) don't need this run again too.
#
...
#
# Re-running it later is safe (e.g. to change MONITOR_MODE) even though it isn't required --
# config edits go inside marked blocks that are replaced, not appended, and every file touched
# is backed up first.
...
BEGIN_MARK="-- >>> Omarchy-Black-Screen-VMware-Fix >>>"
END_MARK="-- <<< Omarchy-Black-Screen-VMware-Fix <<<"
DRY_RUN=no
ASSUME_YES=no
STEP=0
CHANGED=()
